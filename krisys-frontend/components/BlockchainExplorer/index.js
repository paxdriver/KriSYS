...
export default function BlockchainExplorer() {
	...
	const [expandedTxs, setExpandedTxs] = useState({}) // transaction_id -> bool
		...
		const loadData = async () => {
			...
			try {
				...
				if (canonicalBlocks.length !== rawBlocks.length) {
					console.warn(`Discarded ${rawBlocks.length - canonicalBlocks.length} unverified blocks in explorer`)
				}
	...
	const handleSearch = (e) => {
		...
		if (searchType === 'block') {
			...
			nextBlocks = allBlocks.filter((block) =>
				block.transactions.some((tx) => String(tx.transaction_id).includes(term))
			)
			...
			nextBlocks = allBlocks.filter((block) =>
				block.transactions.some((tx) => {
					...
					return fields.some((f) => f ? String(f).toLowerCase().includes(lower) : false)
				})
			)
		}
	...
	const expandAll = () => {
		const map = {}
		filteredBlocks.forEach((b) => { map[b.block_index] = true })
		...
		<div className="explorer-container">
			...
			<div className="explorer-content">
				...
				<form className="explorer-search" onSubmit={handleSearch}>
					...
					<button type="button" className="explorer-btn secondary"
				...
				<div className="explorer-controls">
					...
					<div className="explorer-controls-buttons">
						<button type="button" className="explorer-btn small"
							onClick={expandAll}
					...
					<div className="card explorer-card">
						...
						<div className="card-body">
							{filteredBlocks
								...
								.reverse()
								.map((block) => {
									const isExpanded = expandedBlocks[block.block_index] || false
									const txCount = block.transactions.length
									return (
										<div key={block.block_index} className="block-item explorer-block">
											...
											{isExpanded && txCount > 0 && (
												<div className="block-transactions-list">
													{block.transactions.map((tx) => {
														const isTxExpanded = expandedTxs[tx.transaction_id] || false
														...
														return (
															<div key={tx.transaction_id} className="transaction-item">
																...
																{/* onClick here to toggle message data */}
																<div className="tx-body clickable" onClick={() => toggleTx(tx.transaction_id)}>
																	<div>
																		station:{' '}
																		...
																		tx.related_addresses.length > 0 && (
																			<div>
																				...
																				{tx.related_addresses.map(
																					(addr) => (
																						<code key={addr} className="tx-address">
																							{addr}
																						</code>
																					)
																				)}
																	...
																	<div>
																		created:{' '}
																		{new Date(tx.timestamp_created * 1000).toLocaleString()}
																	</div>
																	<div>
																		posted:{' '}
																	...
																	{/* Conditionally render message data */}
																	{isTxExpanded && tx.message_data && (
																		<div className="tx-message-data">
																			<span className="label">Encrypted Data:</span>
																			<pre>{tx.message_data}</pre>
																		</div>
																	)}

																	<div className="tx-toggle-hint">
																		{isTxExpanded ? 'Click to hide' : 'Click to view encrypted message'}
																	</div>
																</div>
															</div>
														)
