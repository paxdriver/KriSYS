...
class PolicySystem:
	"""
	PolicySystem defines the operational rules of a crisis blockchain.

	IMPORTANT:
	- priority_level values are integers; lower numbers mean higher priority.
	- The semantic names here are documentation + defaults only.
	- Providers may add additional priority levels (6, 7, 8, ...) later
	  without breaking existing chains, as long as they update the policy.
	"""

	# Required policy fields with sane, minimal defaults
	REQUIRED_POLICY_FIELDS = {
		# How often blocks are mined under normal conditions (seconds)
		'block_interval': 180,

		# Maximum serialized transaction size (bytes)
		'size_limit': 5120,

		# Rate limit for non-provider transaction sources (seconds)
		'rate_limit': 180,

		# Priority levels (lower = higher priority)
		
		# These names are descriptive defaults. The integer values are what
		# actually control ordering and mining behavior.
		'priority_levels': {
			'emergency': 1,     # Provider-issued alerts only (immediate mine)
			'operational': 2,   # Check-ins and urgent operational events
			'reserved_3': 3,    # Reserved for provider-defined use
			'reserved_4': 4,    # Reserved for provider-defined use
			'personal': 5,      # Person-to-person messages (best effort)
		},

		# Allowed transaction types in this crisis
		'types': [
			'alert',       # Provider emergency alerts
			'check_in',    # Station or provider check-ins
			'message',     # Encrypted person-to-person messages
		],
	}
	...
	def validate_transaction(self, transaction):
		...
		# DEV NOTE: Priority must be one of the values defined in the active policy. This allows new priorities to be added in future policies without breaking existing chains, while still rejecting unknown values.
		# Priority validation (must be one of the policy-defined values)
		...
		# Priority 1 is reserved for provider-issued emergency alerts.
		# This prevents stations/clients from creating "interrupt" transactions.
		if int(transaction.priority_level) == 1 and transaction.type_field != "alert":
			raise ValueError("Priority 1 is reserved for provider-issued alerts")
...
class Blockchain:
	def __init__(self, policy_system=None):
		...
		# _lock ensures atomicity when adding transactions and mining blocks to avoid inconsistent chain state
	...
	def mine_block(self) -> Block:
		...
		- timestamp_created ASC (older first within priority)
		- transaction_id ASC (tie-break)
		"""
		...
		with self._lock:
			...
			ordered = sorted(self.pending_transactions, key=_sort_key)

			# DEV - Safety check: ensure all pending transactions are valid Transaction objects and have sane values before committing them to an immutable block.
			# !!! ENTIRE CHAIN WILL PERMANENTLY FAIL IF A SINGLE MALFORMED BLOCK FINDS ITS WAY ONTO THE CHAIN !!!
			for tx in ordered:
				if not isinstance(tx, Transaction):
					logger.critical("Invalid transaction type in pending pool; aborting mine")
					raise RuntimeError("Invalid transaction object in pending pool")

				if not isinstance(tx.priority_level, int):
					logger.critical("Transaction has non-integer priority_level; aborting mine")
					raise RuntimeError("Invalid transaction priority")

				if not isinstance(tx.timestamp_created, int):
					logger.critical("Transaction has invalid timestamp_created; aborting mine")
					raise RuntimeError("Invalid transaction timestamp")
			# DEV NOTE: If this validation ever fails in production, capture the offending transaction data and alert the provider immediately. A broken block must never be mined, as it would permanently halt the canonical chain.
